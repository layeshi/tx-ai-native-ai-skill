...
当前授权管理接口支持创建、列出、撤销，不提供原地追加能力的 PATCH 接口。用户需要补充能力时，可创建包含所需能力的新凭证并替换客户端配置；要续接原会话，还需在新授权中选择该 conversationId（如属工作空间，同时选择 workspaceId）。新凭证不能读取旧凭证的任务，续接会话与读取旧 run 是不同权限。是否撤销旧凭证由用户决定。
