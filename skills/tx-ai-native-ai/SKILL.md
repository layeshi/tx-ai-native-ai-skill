...
安装后如需配置连接，可向用户提供默认授权页面 `https://spaceexplorer.cn/agent-access`，但必须先询问用户是否确认打开。用户拒绝时，要求用户输入自己的 Tx-AI 平台根地址（不含 `/api/ai`、不带末尾 `/`），再打开 `<平台根地址>/agent-access`。不得在用户未确认时打开默认网址，也不得猜测或替换用户提供的平台地址。
